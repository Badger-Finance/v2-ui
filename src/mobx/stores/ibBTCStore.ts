...
class IbBTCStore {
  ...
  async mint(mintBalance: TokenBalance, slippage: BigNumber): Promise<TransactionRequestResult> {
    ...
    try {
      ...
      console.error(error);
      queueNotification(`There was an error minting ${this.ibBTC.token.symbol}. Please try again later.`, 'error');
  ...
  private getApprovalMethod(token: Token, spender: string, amount: BigNumber | string = MAX) {
    ...
    return new web3.eth.Contract(settConfig.abi as AbiItem[], token.address).methods.increaseAllowance(spender, amount);
  }
