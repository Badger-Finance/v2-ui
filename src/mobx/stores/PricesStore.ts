...
export default class PricesStore {
  ...
  loadExchangeRates = action(async (): Promise<void> => {
    await retry(async () => {
      const [exchangeRates, bDiggExchangeRates]: [ExchangeRates | null, BDiggExchangeRates | null] = await Promise.all([
        this.getExchangeRates(),
        this.getBdiggExchangeRates(),
      ]);
  ...
  async getBdiggExchangeRates(): Promise<BDiggExchangeRates | null> {
    const url = 'https://api.coingecko.com/api/v3/simple/price?ids=badger-sett-digg&vs_currencies=usd,eth,btc,cad,bnb';
