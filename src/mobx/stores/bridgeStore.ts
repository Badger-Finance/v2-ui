...
class BridgeStore {
    ...
    try {
      await retry(async () => {
        const [badgerBurnFee, badgerMintFee] = (
          await Promise.all([this.adapter.methods.burnFeeBps().call(), this.adapter.methods.mintFeeBps().call()])
  ...
  _getBalances = async (userAddr: string): Promise<void> => {
    ...
    try {
      await retry(async () => {
        const [renbtcBalance, wbtcBalance, byvwbtcBalance, bCRVrenBTCBalance, bCRVsBTCBalance, bCRVtBTCBalance] =
          await Promise.all([
