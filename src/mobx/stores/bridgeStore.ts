...
class BridgeStore {
	...
	constructor(store: RootStore) {
		...
		observe(this.store.wallet as WalletStore, 'provider',
                        ({ newValue, oldValue}: IValueDidChange<provider>) => {
                                if (!newValue) return;
		...
		observe(this.store.wallet as WalletStore, 'connectedAddress',
                        ({ newValue, oldValue}: IValueDidChange<string>) => {
                                ...
                                this.loading = true;
                                Promise.all([
                                        this._fetchTx(newValue),
                                        this._getBalances(newValue),
                                ]).catch((err: Error) => {
		...
		observe(this as BridgeStore, 'current',
                        /*
	...
	_fetchTx = action(async (userAddr: string) => {
                try {
                        await retry(async (context) => {
                                ...
                                let transactions: RenVMTransaction[] = [];
                                results.forEach((doc) => {
                                        transactions.push({
                                                id: doc.id,
                                                ...doc.data(),
                ...
                try {
                        await retry(async (context) => {
                                ...
                                if (recover) {
                                        ...
                                        gateway = this.gjs
                                                .recoverTransfer(provider, parsedTx, parsedTx.id)
                                                .pause();
                                } else {
