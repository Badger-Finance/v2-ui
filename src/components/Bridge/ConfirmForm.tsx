...
export const ConfirmForm = ({
  ...
  const {
    bridge: { renvmMintFee, renvmBurnFee, badgerBurnFee, badgerMintFee, lockNetworkFee, releaseNetworkFee, shortAddr },
    network: { gasPrices },
    uiState: { gasPrice },
  } = store;
      ...
      {values.tabValue <= 1 ? (
        <Grid item xs={12}>
          <div className={classes.checkboxContainer}>
            ...
            <div>
              I acknowledge this transaction requires ETH and that user errors such as sending funds to the wrong
              address or attempting multiple deposits with multiple transactions can result in lost funds
            </div>
          </div>
          {values.spacer}
          <div className={classes.checkboxContainer}>
            <div>
              <Checkbox checked={agreement.userError} onChange={handleCheckbox} name={'userError'} color="primary" />{' '}
            </div>
            <div>
              I acknowledge that the Bitcoin Gateway address in the next screen can only be used once and is valid only
              for 24 hours.
            </div>
          </div>
        </Grid>
