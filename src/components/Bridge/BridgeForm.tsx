...
const UserCancelTx = () => {
  ...
  return (
    <div className={classes.formContainer}>
      ...
      <Dialog open={open} onClose={handleClose} aria-labelledby="cancel-title" aria-describedby="cancel-description">
        <DialogTitle id="cancel-title">{'Cancel transaction?'}</DialogTitle>
        <DialogContent>
          <DialogContentText id="cancel-description">
            Are you sure you wish to cancel this transaction? Please do not cancel if you have already sent funds to the
            provided address.
          </DialogContentText>
        </DialogContent>
        <DialogActions>
  ...
  const pageSwitcher = () => {
    switch (step) {
      ...
      case 3:
        return (
          <Grid item xs={12} className={classes.cardContainer}>
            <SuccessForm values={values} classes={classes} updateState={updateState} resetState={resetState} />
          </Grid>
        );
      default:
        return <div></div>;
