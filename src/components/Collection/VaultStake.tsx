...
export const VaultStake = observer((props: any) => {
	...
	const onSubmit = (params: any) => {
		const amount = new BigNumber(params.amount).multipliedBy(1e18);
		depositAndStake(uiStats.geyser, amount, uiStats.anyWrapped);
	};
	...
	const renderAmounts = (
		<ButtonGroup size="small" className={classes.button} disabled={!connectedAddress}>
			<Button
				...
				variant={!!canDeposit && watch().amount === uiStats.availableFull[25] ? 'contained' : 'outlined'}
				color="primary"
			>
				25%
			</Button>
			<Button
				...
				variant={!!canDeposit && watch().amount === uiStats.availableFull[50] ? 'contained' : 'outlined'}
				color="primary"
			>
				50%
			</Button>
			<Button
				...
				variant={!!canDeposit && watch().amount === uiStats.availableFull[75] ? 'contained' : 'outlined'}
				color="primary"
			>
				75%
			</Button>
			<Button
				...
				variant={!!canDeposit && watch().amount === uiStats.availableFull[100] ? 'contained' : 'outlined'}
				color="primary"
			>
				100%
			</Button>
	...
	// const anyAvailable = !!uiStats.availableBalance && parseFloat(uiStats.availableBalance) !== 0;
	return (
		<>
			<DialogContent>
				<div
				...
				>
					{uiStats.anyWrapped ? 'Stake' : 'Deposit'}
				</Button>
			</DialogContent>
