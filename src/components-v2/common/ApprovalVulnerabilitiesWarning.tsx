    ...
    return (
      <li key={vulnerability.transactionId}>
        <Typography variant="body1">
          {asset.token.symbol}:{' '}
          <Link target="_blank" rel="noreferrer" href={`https://etherscan.io/tx/${vulnerability.transactionId}`}>
            See on Etherscan
          </Link>
        </Typography>
      </li>
        ...
        <ol className={classes.list}>
          <li>
            <Typography variant="body1">
              ...
              <Link target="_blank" rel="noreferrer" href={`https://debank.com/profile/${onboard.address}/approve`}>
                {`https://debank.com/profile/${onboard.address}/approve`}
              </Link>{' '}
            </Typography>
          </li>
          <li>
            <Typography variant="body1">{`Search for the hacker's address: ${EXPLOIT_HACKER_ADDRESS}`}</Typography>
          </li>
          <li>
