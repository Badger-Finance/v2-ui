  ...
  return (
    <Box component={Paper} className={classes.tooltipRoot}>
      ...
      {bcvxCrv > 0 && (
        <div className={classes.tooltipItem}>
          <Typography variant="body2">
            {numberWithCommas(bcvxCrvPerHundred.toFixed(2))} <span className={classes.bcvxCrvAmount}>bcvxCrv</span>
            <span> per 100 bveCVX</span>
          </Typography>
          <Typography variant="caption" className={classes.subItem}>
      ...
      <Typography variant="caption">
        Value per 100 bveCVX: ${numberWithCommas(totalValue.toFixed())} ({numberWithCommas(apr.toFixed(2))}% APR)
      </Typography>
    </Box>
  );
};
