...
import { formatBalance, formatBalanceUnderlying, formatBalanceValue, formatGeyserBalance, formatGeyserBalanceValue, formatPrice, formatTokenBalanceValue } from "mobx/reducers/statsReducers";
...
const UserListDisplay = observer((props: UserListDisplayProps) => {
  ...
  const walletListItems = settList.map((sett) => {
  ...
  const depositListItems = settList.map((sett) => {
    const vault = vaults[sett.vaultToken];
    if (!vault) {
      return null;
    }
  ...
  const vaultListItems = settList.map((sett) => {
    const vault = vaults[sett.vaultToken];
  ...
  return (
    <>
      {displayWallet && <SettTable title={`Your Wallet - ${walletBalance}`} tokenTitle={'Available'} classes={classes} period={period} settList={walletListItems} />}
      {displayDeposit && <SettTable title={`Your Vault Deposits - ${depositBalance}`} tokenTitle={'Available'} classes={classes} period={period} settList={depositListItems} />}
      {displayVault && <SettTable title={`Your Staked Amounts - ${vaultBalance}`} tokenTitle={'Available'} classes={classes} period={period} settList={vaultListItems} />}
