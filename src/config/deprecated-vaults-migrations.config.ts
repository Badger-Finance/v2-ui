...
const migratingConfig: Config = {
  [Network.Ethereum]: {
    ...
    [MAINNET_VAULTS['native.sushiibBTCwBTC']]:
      MAINNET_VAULTS['native.ibbtcCrv'],
    [MAINNET_VAULTS['native.uniBadgerWbtc']]:
      MAINNET_VAULTS['native.badgerCrv'],
    ...
    [MAINNET_VAULTS['native.uniDiggWbtc']]:
      MAINNET_VAULTS['native.sushiDiggWbtc'],
    [MAINNET_VAULTS['native.tricryptoCrv']]:
      MAINNET_VAULTS['native.tricryptoCrv2'],
  },
};

export const DEPRECATED_VAULTS_MIGRATIONS_MAPPING = new Map(
  Object.entries(migratingConfig).map(([networkId, vaults]) => [
    networkId,
    new Map(Object.entries(vaults)),
  ]),
);
