...
export function getWeb3ModalProviders(config: NetworkConfig) {
  ...
  // const network = Object.values(CHAIN_DATA_LIST).find((network) => network.chainId === config.chainId);
  ...
  return {
    ...
    'custom-example': {
      ...
      options: {
        projectId: 'dcdb98454f70814fa8bf6983dca8b5a9', // required
        chains: [1], // required
        showQrModal: true, // requires @walletconnect/modal
      },
      connector: async (ProviderPackage: typeof EthereumProvider, options: any) => {
        console.log('custom provider');
        const provider = await ProviderPackage.init(options);
        return provider;
      },
    },
  };
