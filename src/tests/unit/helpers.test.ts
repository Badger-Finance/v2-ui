...
describe('inCurrency', () => {
	test.each([
		...
		[new BigNumber(12.5678), 'btc', true, 9, undefined, `₿ ${12.5678 * exchangeRates.btc}000`],
		[new BigNumber(-12.5678), 'btc', false, 9, undefined, `₿ -${12.5678 * exchangeRates.btc}000`],
	...
	])('inCurrency(%f, %s, %s, %i, %s) returns %s', (value, currency, hide, preferredDecimals, noCommas, expected) => {
...
describe('formatWithDecimals', () => {
	...
	])('formatWithDecimals(%f, %i) returns %s', (value, decimals, expected) => {
		expect(formatWithDecimals(value, decimals)).toBe(expected);
	});
});
