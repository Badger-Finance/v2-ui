...
describe('Boost Ranks Utils', () => {
  ...
  describe('getHighestRankFromStakeRatio', () => {
    ...
    test.each(inputTable)(
      'getHighestRankFromStakeRatio(%p) returns %p',
      (stakeRatio, _badgerType, result) => {
        expect(getHighestRankFromStakeRatio(stakeRatio as number)).toEqual(
          result,
        );
      },
    );
  ...
  describe('getNextBoostRank', () => {
    const inputTable = [
      [
        BadgerType.Basic,
        BadgerType.Neo,
        BADGER_TYPE_BOOSTS.basic,
        BADGER_TYPE_BOOSTS.neo,
      ],
      [
        BadgerType.Neo,
        BadgerType.Hero,
        BADGER_TYPE_BOOSTS.neo,
        BADGER_TYPE_BOOSTS.hero,
      ],
      [
        BadgerType.Hero,
        BadgerType.Hyper,
        BADGER_TYPE_BOOSTS.hero,
        BADGER_TYPE_BOOSTS.hyper,
      ],
      [
        BadgerType.Hyper,
        BadgerType.Basic,
        BADGER_TYPE_BOOSTS.hyper,
        BADGER_TYPE_BOOSTS.frenzy,
      ],
      [
        BadgerType.Hyper,
        BadgerType.Hyper,
    ...
    test.each(inputTable)(
      'getNextBoostRank(of:%s) returns %p',
      (_name, _toName, fromRank, toRank) => {
  ...
  describe('calculateUserBoost', () => {
    ...
    test.each(inputTable)(
      'calculateUserBoost(stakeRatio:%p) returns %p',
      (stakeRatio, userBoost) => {
  ...
  describe('calculateNativeToMatchBoost', () => {
    ...
    test.each(inputTable)(
      ...
      (native, nonNative, desiredRank) => {
        ...
        expect(
          getHighestRankFromStakeRatio(stakeRatio).stakeRatioBoundary,
        ).toEqual((desiredRank as BoostRank).stakeRatioBoundary);
      },
    );
  });
